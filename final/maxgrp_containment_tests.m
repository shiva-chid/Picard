...
*/
    ...
    while not IsEof(s) do
        ...
        p := s_split[1]; ap := s_split[2]; bp := s_split[3]; cp := s_split[4];
        Append(~charpols,<p,P![p^3,p^2*ap,p*bp,cp,bp,ap,1]>);
        s := Gets(fil);
    end while;
    delete(fil);
...
intrinsic Bound(pandcharpol :: Tup, alpha :: RngElt) -> RngCycElt
...
intrinsic C1test(C :: CrvHyp : radical_cond := 1, primes_bound := 500, charpols := [], F := Rationals()) -> SeqEnum
{Given a hyperelliptic curve C of genus 3 having an imaginary quadratic field F as its geometric
endomorphism algebra, returns a list of primes which contains all primes ell at which the mod-ell
Galois image associated to the Jacobian of C, is contained in the C1-type maximal subgroup of
GL(3,F_ell), i.e., the reducible type maximal subgroup. These primes are the ones that divide the
Billerey Bounds associated to all good primes splitting in F, that are considered. The optional 
parameters are
1. some integer divisible by all the bad primes of the curve.
2. a bound for how many primes to consider.
3. a precomputed list of tuples consisting of a prime p and the characteristic polynomial of Frobenius at p,
for various primes.}
    ...
    OF := RingOfIntegers(F);
    HecGrpF := HeckeCharacterGroup(AbelianExtension(F)); chi := HecGrpF.1;
    if radical_cond eq 1 then radical_cond := &*BadPrimes(C); end if;
    if charpols eq [] then charpols := getcharpols(C : primesend := primes_bound); end if;
    Bignum := 0;
    for pandcharpol in charpols do
        p := pandcharpol[1];
        charpol := pandcharpol[2];
        if (chi(p) ne 1) or (radical_cond mod p eq 0) then continue; end if;
        pFacs := PrimeIdealsOverPrime(F,p);
        for pfac in pFacs do
            tf, a := IsPrincipal(pfac);
//            printf "pandcharpol = %o, a = %o\n", pandcharpol, a;
            B := Bound(pandcharpol,a);
            Bignum := GCD(Bignum,p*Norm(B)); // The bound associated to B_p doesn't give us any information on p.
            if Bignum eq 2^Valuation(Bignum,2)*3^Valuation(Bignum,3) then
                return PrimeFactors(Bignum);
...
for various primes.}
    ...
    CubicHeckeG := cubic_fields(radical_cond,F);
//    print #CubicHeckeG;
...
//                    if ap mod ell ne 0 and not IsSquare(resmodell(disc_charpolp)) then
                    ...
                    // Note: the following condition asks for fp to be reducible, but not to be of the forms (T-a)^3 or T^3-a
...
intrinsic C3test2(C :: CrvHyp : radical_cond := 1, primes_bound := 500, charpols := [], F := Rationals()) -> SeqEnum
{Given a hyperelliptic curve C of genus 3 having an imaginary quadratic field F as its geometric
endomorphism algebra, returns a list of primes which contains all primes ell at which the mod-ell
Galois image associated to the Jacobian of C, is contained in the C3-type maximal subgroup of
GL(3,F_ell), i.e., the field-extension type maximal subgroup. The optional parameters are
1. some integer divisible by all the bad primes of the curve.
2. a bound for how many primes to consider.
3. a precomputed list of tuples consisting of a prime p and the characteristic polynomial of Frobenius at p,
for various primes.}
...
/*
    factored_charpols := [(chi(x[1]) eq 1) select <x[1],Factorisation(ChangeRing(x[2],F))> else <x[1],Factorisation(ChangeRing(Bracket(2,x[2]),F))> : x in charpols];
    primes_F := [(chi(x[1]) eq 1) select PrimeIdealsOverPrime(F,x[1]) else [x[1]*OF] : x in charpols];
    genvals := [[[mu3toZmod3(&*[(v[i] ne 0) select matrix_of_vals[j,i]^(v[i] mod 3) else 1 : i in [1..#v]]) : j in [1..n]] where v is Eltseq(x @@ pi) : x in p] : p in primes_F];
*/
    aps := [];
    mat := Matrix(GF(3),n,1,[0 : i in [1..n]]);
...
for various primes.}
    ...
    quaddirichletchars := quad_fields(radical_cond,F : all := false);
    possiblequadchars := [];
...
for various primes.}
    ...
    QuadHeckeG := quad_fields(radical_cond,F);
...
intrinsic C2andC3test2(C :: CrvHyp : radical_cond := 1, primes_bound := 500, charpols := [], F := Rationals()) -> SeqEnum, SeqEnum
{Given a hyperelliptic curve C of genus 3 having an imaginary quadratic field F as its geometric
endomorphism algebra, returns two lists of primes.
1. The first list contains all primes ell at which the mod-ell Galois image associated to the Jacobian of C,
is contained in the C3-type maximal subgroup of GL(3,F_ell) or GU(3,F_ell), i.e., the field-extension
type maximal subgroup.
2. The second list contains all primes ell at which the mod-ell Galois image is contained in the C2-type
maximal subgroup of GL(3,F_ell) or GU(3,F_ell), i.e., the imprimitive type maximal subgroup.
The optional parameters are
1. some integer divisible by all the bad primes of the curve.
2. a bound for how many primes to consider.
3. a precomputed list of tuples consisting of a prime p and the characteristic polynomial of Frobenius at p,
for various primes.}
    ...
    charpols := [x : x in charpols | x[1] gt 3];
    try
        C3primes := C3test2(C : radical_cond := radical_cond, primes_bound := primes_bound, charpols := charpols, F := F);
    catch e;
        C3primes := [];
        require C3primes ne [] : "C3test fails.";
    end try;
